...
class ServerTest(TestCase):
    ...
    # def test_text(self):
    #     event_id = self.opbeat_python.capture('Message', message='hello')

    #     self.assertEquals(Group.objects.count(), 1)
    #     self.assertEquals(Event.objects.count(), 1)

    #     message = Event.objects.get()
    #     self.assertEquals(message.client_supplied_id, event_id)
    ...
    #     event_id = self.opbeat_python.capture('Exception')
