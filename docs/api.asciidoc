...
[float]
[[api-set-custom-span-context]]
==== `elasticapm.set_custom_span_context()`

[small]#Added in v6.19.0.#

Attach custom contextual data to the current span.
...
elasticapm.set_custom_span_context({'billing_amount': product.price * item_count})
----
