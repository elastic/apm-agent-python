...
Upgrades between minor versions of the agent, like from 3.1 to 3.2 are always backwards incompatible.
Upgrades that involve a major version bump often come with some backwards incompatible changes.

We highly recommend to always pin the version of `elastic-apm` in your `requirements.txt` or `Pipfile` to avoid automatic upgrades to potentially incompatible versions.
