...
You need an APM server where the APM data will be sent to. {apm-guide-ref}/apm-quick-start.html[Set up an APM Server] if you have not not done so yet. For the best-possible performance we recommend to setup APM on Elastic Cloud in the _same AWS region_ as your AWS Lambda functions. 
