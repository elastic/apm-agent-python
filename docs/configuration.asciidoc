...
Note that this option doesn't have anything to do with logging handlers.
...
The API key must have been previously created using the APM Server command line tool. Please see the APM Server
documentation for details on how to create this.
...
This setting allows you to limit the length of resulting string.
...
This setting allow you to modify the number of lines of source code that are being collected. 
...
Please see our documentation on <<sanitizing-data, sanitizing data>> for more information on how to
filter this data.
...
Setting an upper limit will prevent overloading the agent and the APM Server with too much work.
...
By setting this to `False`, this metric tracking is completely disabled, which can reduce the overhead of the agent.
...
The unit is provided as a suffix directly after the number without and separation by whitespace.
