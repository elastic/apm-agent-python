...
[float]
[[config-transaction-mark-errors]]
==== `transaction_mark_errors`

|============
| Environment                           | Django/Flask              | Default
| `ELASTIC_APM_TRANSACTION_MARK_ERRORS` | `TRANSACTION_MARK_ERRORS` | `all`
|============

Possible values: `handled`, `unhandled`, `all`, `off`

Mark errors that happen during a transaction on the transaction.
