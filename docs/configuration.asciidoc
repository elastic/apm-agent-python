...
By default, the APM agent will collect a stack trace with every recorded span
which has a duration equal to or longer than this configured threshold.  While
...
threshold to ensure that you are only collecting stack traces for spans which
could be problematic.

To collect traces for all spans, independent of the length, set the value to `0`.
