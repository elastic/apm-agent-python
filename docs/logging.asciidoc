...
The {ecs-logging-python-ref}/intro.html[`ecs-logging-python`] library can also be used to use the ECS logging format without an APM agent.
...
The agent can automatically reformat application logs to ECS format, without adding a dependency to `ecs-logging-python`, modifying the application
